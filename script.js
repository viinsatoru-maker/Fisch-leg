...
const gameData = {
    ...
    // ==================== NEW: SECRET TOKENS ====================
    secretTokens: 0 // Token untuk ditukar di dungeon
};
...
const pets = [
    ...
    { id: 3, name: "Ayam Perfect", description: "PERFECT CATCH 100% setiap mancing! (tanpa minigame)", effect: { type: "perfect_chance", value: 100 }, price: 10000, currency: "coins", emoji: "🐓" },
...
// ==================== SKILL TREE DATA (UPDATED with EXPERT) ====================
const skillTree = {
...
// ==================== SKILL TREE (UPDATED with EXPERT) ====================
...
function addGamepassExp(amount) {
    ...
    while (Number(gameData.gamepass.exp) >= currentLevelReq && gameData.gamepass.level < 20) {
...
// ==================== FISHING SYSTEM (UPDATED with Pet Ayam) ====================
...
// ==================== startFishing (UPDATED with Pet Ayam) ====================
...
function finishFishing(fish, perfectCatch = false) {
    ...
    if (gameData.gamepass.owned) {
        // Jika ada pet Ayam (perfect_chance) tidak menambah EXP Gamepass, tapi Expert Skill tetap jalan
        if (activePet && activePet.effect.type === "exp_bonus") {
            addGamepassExp(activePet.effect.value);
        }
        // Expert Skill sudah menambah EXP Gamepass melalui expGained di atas
    }
...
// ==================== DUNGEON FUNCTIONS (UPDATED with TOKEN REWARDS) ====================
...
function switchToDungeon() {
    ...
    loadDungeonShop();
    loadDungeonLevels();
    loadTokenExchange(); // Load token exchange menu
    updateDungeonStats();
}
...
function updateDungeonStats() {
    ...
    if (tokenSpan) {
        tokenSpan.textContent = gameData.secretTokens || 0;
    }
}
