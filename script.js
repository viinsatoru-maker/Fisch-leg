// ==================== GAME DATA ====================
const gameData = {
    version: 3, // Naikkan versi karena ada fitur baru!
    coins: 500,
    diamonds: 0,
    exp: 0,
    level: 1,
    ...
    atlantis: {
        active: false,
        nextSpawn: Date.now() + (10 * 60 * 1000), // 10 menit dari sekarang
        disappearTime: null,
        spotId: 7,
        lastNotification: null
    },
    // NEW: Favorite Fish System
    favoriteFish: [] // Array of fish indices that are favorited
};

// ==================== VERSION CONTROL ====================
const GAME_VERSIONS = {
    current: 3,
    latest: 3, // Naikkan ke 3
...
const atlantisSpot = {
    ...
    isEventSpot: true,
    fishes: [
        // LEGENDARY (70% chance) - Dolphin
        { id: 700, name: "Dolphin", rarity: "legendary", chance: 70, price: 5000, emoji: "🐬", spot: "atlantis" },
        
        // MYTHICAL (29% chance) - Whale
        { id: 701, name: "Whale", rarity: "mythical", chance: 29, price: 15000, emoji: "🐋", spot: "atlantis" },
        
        // SPECIAL (1% chance) - Neptunus (rarity ???)
        { id: 702, name: "Neptunus", rarity: "special", chance: 1, price: 50000, emoji: "🧜‍♂️", spot: "atlantis" }
    ]
};
...
function checkAtlantisSpawn() {
    ...
    if (!gameData.atlantis.active && now >= gameData.atlantis.nextSpawn) {
        ...
        gameData.atlantis.disappearTime = now + (10 * 60 * 1000); // 10 menit
        gameData.atlantis.nextSpawn = now + (10 * 60 * 1000); // 10 menit lagi untuk spawn berikutnya
        
        // Tambahkan spot Atlantis ke daftar spot yang tersedia
        ...
        showNotification("🌊 ATLANTIS TERBUKA! Spot misterius muncul selama 10 menit!", "success");
    ...
    if (gameData.atlantis.active && now >= gameData.atlantis.disappearTime) {
        ...
        // Hapus spot Atlantis dari daftar
        ...
        // Notifikasi
        showNotification("🌊 Atlantis menghilang... Kembali dalam 10 menit!", "info");
    }
}

// ==================== NEW: MYSTERY BOX FUNCTIONS ====================
    ...
    luckDisplay.innerHTML = `
        <div style="text-align: center; padding: 15px; background: rgba(0,0,0,0.2); border-radius: 8px; margin: 15px 0;">
            ...
            <div style="font-size: 0.9rem; text-align: left;">
                ...
                ${gameData.atlantis.active ? '<div style="color: #00ffff;">🌊 Atlantis aktif!</div>' : ''}
            </div>
        </div>
    `;
}
...
function updateGachaTab() {
    ...
    // Hapus tombol yang ada sebelumnya biar ga double
    ...
    // Secret Gacha button
    if (!document.getElementById('secret-gacha-btn')) {
...
function loadSellItems() {
    ...
    // Tampilkan pesan jika semua ikan favorit
...
function sellSelectedFish() {
    ...
    // Cek apakah ada ikan favorit yang dipilih
    ...
    // Update favoriteFish indices setelah penghapusan
...
function sellAllFish() {
    ...
    gameData.coins += totalSold;
    gameData.backpack = [];
    gameData.favoriteFish = []; // Kosongkan favorit
                ...
                <div style="display: flex; justify-content: space-between; align-items: center;">
                    <div style="flex: 1;">
                        ...
                        ${recipe.input.map(item => `
                            <div style="display: flex; align-items: center; gap: 10px; margin: 5px 0;">
                                <span style="font-size: 1.5rem;">📦</span>
                                <span style="color: white;">${item.quantity}x ${item.name || item.rarity || (item.type === 'coin' ? 'Coin' : '')}</span>
                            </div>
                        `).join('')}
                    </div>
...
function showRankBattleSelection(opponent) {
    ...
    let fishOptions = '';
    gameData.backpack.forEach((fish, index) => {
        // Skip favorite fish? Boleh pilih favorit buat tarung
        fishOptions += `
...
function selectFishForBattle(fishIndex, opponentPrice) {
    ...
    // Hitung chance menang
    const priceDifference = fish.price - opponentPrice;
    let winChance = 50 + (priceDifference / 50); // +2% per 100 koin selisih
...
function processBattleResult(isWin, fish, winChance) {
    ...
    if (isWin) {
        ...
        // Hitung bonus streak
        const streakBonus = Math.floor(gameData.rank.wins / 10) * 0.1; // +10% setiap 10 win
...
function loadAquarium() {
    ...
    loadFishTank('special', 'special-tank'); // NEW: untuk Neptunus
}
