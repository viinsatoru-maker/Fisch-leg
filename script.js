...
// ==================== EXCHANGE RECIPES (TANPA RECIPE MINING) ====================
...
function loadMiningExchange() {
    ...
    // Mining exchange recipes (hanya untuk menu mining)
    const miningExchangeRecipes = [
        { 
            id: 101, 
    ...
    miningExchangeRecipes.forEach(recipe => {
        const recipeCard = document.createElement('div');
        recipeCard.className = 'exchange-card';
        
        let canCraft = false;
        if (recipe.input.type === 'coin') {
            canCraft = Number(gameData.coins) >= recipe.input.quantity;
        } else if (recipe.input.type === 'rock') {
            canCraft = (gameData.mining.rocks || 0) >= recipe.input.quantity;
        }
                ...
                <div style="display: flex; justify-content: space-between; align-items: center;">
                    <div style="flex: 1;">
                        ...
                        <div style="display: flex; align-items: center; gap: 10px; margin: 5px 0;">
                            <span style="font-size: 1.5rem;">${recipe.input.emoji}</span>
...
function exchangeMiningItems(recipe) {
    ...
    updateUI();
    updateMiningStats();
    loadMiningExchange(); // Refresh tampilan
}
...
function saveGame() {
    try {
        ...
        localStorage.setItem('fishLegSave', saveString);
        
        // Verifikasi save berhasil
        const saved = localStorage.getItem('fishLegSave');
        if (saved) {
            console.log("✅ Game saved successfully! Size:", saved.length, "bytes");
            console.log("📊 Data:", {
            ...
            console.error("❌ Save failed - localStorage empty after save!");
        }
        
    } catch (error) {
        console.error("❌ Save error:", error);
        showNotification("❌ Gagal menyimpan game: " + error.message, "error");
    }
}
...
function setupEventListeners() {
    ...
    window.addEventListener('click', (e) => {
        ...
        if (miningResultModal && e.target === miningResultModal) miningResultModal.style.display = 'none';
    });
