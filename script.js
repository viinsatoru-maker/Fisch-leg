...
const GAME_VERSIONS = {
    ...
    features: {
        ...
        16: "⛏️ MINING UPDATE! Sistem mining dengan rock, tools, dan skill baru!"
    }
};
...
const exchangeRecipes = [
    ...
    { id: 14, name: "Coin to Diamond", description: "Tukar 800 Coin dengan 1 Diamond", input: [ { type: "coin", quantity: 800 } ], output: { type: "diamond", quantity: 1, emoji: "💎" } },
    { id: 15, name: "Coin to Diamonds", description: "Tukar 70.000 Coin dengan 100 Diamond", input: [ { type: "coin", quantity: 70000 } ], output: { type: "diamond", quantity: 100, emoji: "💎" } },
    { id: 16, name: "Rock to Diamond", description: "Tukar 5 Rock dengan 1 Diamond", input: [ { type: "rock", quantity: 5 } ], output: { type: "diamond", quantity: 1, emoji: "💎" } },
    { id: 17, name: "Rock to Diamonds", description: "Tukar 400 Rock dengan 100 Diamond", input: [ { type: "rock", quantity: 400 } ], output: { type: "diamond", quantity: 100, emoji: "💎" } }
];
...
let resultModal, gachaModal, exchangeModal, rankModal, dungeonModal, battleModal, miningModal;
...
function initGame() {
    ...
    showNotification("🎮 Game loaded successfully! Cari Miner Helm & Flashlight untuk buka Mining!", "success");
}
...
function createMiningButton() {
    ...
    if (document.getElementById('mining-menu-btn')) return;
    
    const dungeonBtn = document.getElementById('dungeon-menu-btn');
    if (!dungeonBtn) return;
    
    const miningBtn = document.createElement('button');
    miningBtn.id = 'mining-menu-btn';
    miningBtn.className = 'menu-btn';
    miningBtn.innerHTML = '⛏️ MINING';
    miningBtn.style.background = 'linear-gradient(45deg, #8B4513, #D2691E)';
    miningBtn.style.marginTop = '10px';
    miningBtn.style.width = '100%';
    miningBtn.style.padding = '12px';
    miningBtn.style.border = 'none';
    miningBtn.style.borderRadius = '25px';
    miningBtn.style.color = 'white';
    miningBtn.style.fontWeight = 'bold';
    miningBtn.style.fontSize = '1.2rem';
    miningBtn.style.cursor = 'pointer';
    miningBtn.style.transition = 'all 0.3s';
    ...
    if (miningBtn) {
        miningBtn.addEventListener('click', switchToMining);
    }
}
...
function updateMiningStats() {
    const rocksSpan = document.getElementById('mining-rocks');
    const totalMinesSpan = document.getElementById('mining-total-mines');
    const totalCoinsSpan = document.getElementById('mining-total-coins');
    const totalDiamondsSpan = document.getElementById('mining-total-diamonds');
    const perfectCountSpan = document.getElementById('mining-perfect-count');
    
    if (rocksSpan) rocksSpan.textContent = gameData.mining.rocks || 0;
    if (totalMinesSpan) totalMinesSpan.textContent = gameData.mining.stats.totalMines || 0;
    if (totalCoinsSpan) totalCoinsSpan.textContent = gameData.mining.stats.totalCoins || 0;
    if (totalDiamondsSpan) totalDiamondsSpan.textContent = gameData.mining.stats.totalDiamonds || 0;
    if (perfectCountSpan) perfectCountSpan.textContent = gameData.mining.stats.perfectCount || 0;
}

function loadMiningMain() {
    const miningContainer = document.getElementById('mining-main-content');
    ...
    const totalLuck = currentTool.luck * (1 + luckyLevel);
    
    miningContainer.innerHTML = `
        <div style="text-align: center; padding: 20px;">
            <div style="background: rgba(0,0,0,0.3); border-radius: 15px; padding: 20px; margin-bottom: 20px;">
                <h3 style="color: #FFD700; margin-bottom: 15px;">⛏️ MINING AREA</h3>
                <div style="display: flex; justify-content: space-around; margin-bottom: 20px;">
                    <div>
                        <div style="color: #00ffff;">Tool</div>
                        <div style="font-size: 2rem;">${currentTool.emoji}</div>
                        <div style="color: white;">${currentTool.name}</div>
                        <div style="color: #4CAF50;">Luck: ${currentTool.luck}x</div>
                    </div>
                    <div>
                        <div style="color: #00ffff;">Total Luck</div>
                        <div style="font-size: 2rem;">${totalLuck.toFixed(1)}x</div>
                        ${luckyLevel > 0 ? `<div style="color: #4CAF50;">Skill: +${luckyLevel*100}%</div>` : ''}
                    </div>
                </div>
                ${hasPerfectCut ? '<div style="color: #FFD700; margin-bottom: 10px;">✨ Perfect Cut Aktif! Auto Perfect!</div>' : ''}
            </div>
            
            <div id="mining-rock" style="cursor: pointer; transition: transform 0.1s;" onclick="startMining()">
                <div style="font-size: 10rem; animation: float 3s infinite ease-in-out;">🪨</div>
                <p style="color: #ccc; margin-top: 20px;">Klik batu untuk mining!</p>
            </div>
            
            <div id="mining-minigame-indicator" style="display: none; margin-top: 30px;">
                <div style="background: rgba(0,0,0,0.5); padding: 15px; border-radius: 10px;">
                    <p style="color: #FFD700; margin-bottom: 10px;">🎯 PERFECT TIMING!</p>
                    <div style="width: 200px; height: 20px; background: #333; border-radius: 10px; margin: 0 auto; position: relative; overflow: hidden;">
                    ...
                    <p style="color: #ccc; margin-top: 10px;">Klik saat jarum di area emas!</p>
                </div>
            </div>
        </div>
    `;
...
function checkMiningMinigameResult() {
    const needle = document.getElementById('mining-needle');
...
function finishMining(perfectCatch = false) {
...
function showMiningResult(emoji, name, amount, perfectCatch) {
    const miningModal = document.getElementById('mining-result-modal');
    const miningResultContent = document.getElementById('mining-result-content');
    
    if (!miningModal || !miningResultContent) return;
...
function loadMiningShop() {
    const shopContainer = document.getElementById('mining-shop-items');
    if (!shopContainer) return;
    
    shopContainer.innerHTML = '';
    
    miningTools.forEach(tool => {
        const isOwned = tool.owned;
        const isEquipped = tool.id === gameData.mining.currentTool;
        const canAfford = Number(gameData.coins) >= tool.price;
        ...
        toolCard.innerHTML = `
            <div style="background: rgba(255,255,255,0.05); padding: 15px; border-radius: 8px; margin: 5px; text-align: center;">
                <div style="font-size: 3rem; margin-bottom: 10px;">${tool.emoji}</div>
                <div style="font-weight: bold; color: white; margin-bottom: 5px;">${tool.name}</div>
                <div style="color: #00ffff; font-size: 13px;">+${tool.luck}x Luck</div>
                <div style="color: #FFD700; margin: 10px 0;">${tool.price} 🪙</div>
                ${isOwned ? 
                    `<button class="equip-tool-btn" ${isEquipped ? 'disabled' : ''}
                            style="width: 100%; padding: 8px; background: ${isEquipped ? '#666' : '#4CAF50'}; border: none; border-radius: 6px; color: white; cursor: ${isEquipped ? 'not-allowed' : 'pointer'};">
                        ${isEquipped ? '✓ DIGUNAKAN' : '🔧 GUNAKAN'}
                    </button>` :
                    `<button class="buy-tool-btn" ${!canAfford ? 'disabled' : ''}
        ...
        const btn = toolCard.querySelector('.buy-tool-btn, .equip-tool-btn');
        if (btn) {
            btn.addEventListener('click', () => {
                if (isOwned) {
                    equipMiningTool(tool.id);
                } else if (canAfford) {
                    buyMiningTool(tool);
                }
            });
        ...
        shopContainer.appendChild(toolCard);
    });
}
...
function equipMiningTool(toolId) {
...
function loadMiningSkillTree() {
    const skillContainer = document.getElementById('mining-skill-tree');
    if (!skillContainer) return;
    
    skillContainer.innerHTML = '';
    
    const perfectCutCard = createMiningSkillCard('perfectCut', miningSkillTree.perfectCut);
    const luckyCard = createMiningSkillCard('lucky', miningSkillTree.lucky);
    
    skillContainer.appendChild(perfectCutCard);
    skillContainer.appendChild(luckyCard);
}

function createMiningSkillCard(skillKey, skillData) {
    let currentLevel, isUnlocked;
    
    if (skillKey === 'perfectCut') {
        isUnlocked = gameData.mining.skill.perfectCut?.unlocked || false;
        currentLevel = isUnlocked ? 1 : 0;
    } else {
        currentLevel = gameData.mining.skill.lucky?.level || 0;
    }
    ...
    let bonusText = '';
    if (skillKey === 'perfectCut') bonusText = isUnlocked ? 'Auto Perfect' : 'Manual Perfect';
    else if (skillKey === 'lucky') bonusText = `+${currentLevel * 100}% Luck Mining`;
    ...
    card.innerHTML = `
        <div style="text-align: center; background: rgba(255,255,255,0.05); padding: 20px; border-radius: 10px; width: 250px;">
            ...
            ${currentLevel < maxLevel ? `
                ...
                <button class="upgrade-mining-skill-btn" ${!canUpgrade ? 'disabled' : ''}
    ...
    const upgradeBtn = card.querySelector('.upgrade-mining-skill-btn');
    if (upgradeBtn && canUpgrade) {
        upgradeBtn.addEventListener('click', () => upgradeMiningSkill(skillKey, nextPrice, skillData.currency));
    }
    
    return card;
}

function upgradeMiningSkill(skillKey, price, currency = 'coins') {
...
function loadMiningExchange() {
    const exchangeContainer = document.getElementById('mining-exchange-items');
    if (!exchangeContainer) return;
    
    exchangeContainer.innerHTML = '';
    
    const miningRecipes = exchangeRecipes.filter(r => r.id >= 14 && r.id <= 17);
    
    miningRecipes.forEach(recipe => {
        const recipeCard = document.createElement('div');
        recipeCard.className = 'exchange-card';
        
        let canCraft = false;
        if (recipe.input[0].type === 'coin') {
            canCraft = Number(gameData.coins) >= recipe.input[0].quantity;
        } else if (recipe.input[0].type === 'rock') {
            canCraft = (gameData.mining.rocks || 0) >= recipe.input[0].quantity;
        }
                ...
                <div style="display: flex; justify-content: space-between; align-items: center;">
                    <div style="flex: 1;">
                        ...
                        <div style="display: flex; align-items: center; gap: 10px; margin: 5px 0;">
                            <span style="font-size: 1.5rem;">${recipe.input[0].type === 'coin' ? '💰' : '🪨'}</span>
                            <span style="color: white;">${recipe.input[0].quantity}x ${recipe.input[0].type === 'coin' ? 'Coin' : 'Rock'}</span>
                        </div>
                    </div>
...
function exchangeMiningItems(recipe) {
                ...
                <div style="display: flex; justify-content: space-between; align-items: center;">
                    <div style="flex: 1;">
                        ...
                        ${recipe.input.map(item => `
                            <div style="display: flex; align-items: center; gap: 10px; margin: 5px 0;">
                                <span style="font-size: 1.5rem;">📦</span>
                                <span style="color: white;">${item.quantity}x ${item.name || item.rarity || (item.type === 'coin' ? 'Coin' : (item.type === 'diamond' ? 'Diamond' : (item.type === 'rock' ? 'Rock' : '')))}</span>
                            </div>
                        `).join('')}
                    </div>
...
function checkExchangeRequirements(recipe) {
    for (const requirement of recipe.input) {
        ...
        } else if (requirement.type === "rock") {
            count = Number(gameData.mining.rocks || 0);
        }
...
function exchangeItems(recipe) {
    ...
    const backupRocks = Number(gameData.mining.rocks || 0);
    ...
    try {
        for (const requirement of recipe.input) {
            if (requirement.type === "fish") {
                ...
                if ((gameData.mining.rocks || 0) < Number(requirement.quantity)) {
                    throw new Error("Rock tidak cukup");
                }
                gameData.mining.rocks -= Number(requirement.quantity);
            }
        }
        ...
        if (recipe.output.type === "fish") {
            ...
            addToAquarium({ id: 9999, name: "💍 One Ring", emoji: "💍", rarity: "special" });
            ...
            addToAquarium({ id: 9998, name: "🔦 Flashlight", emoji: "🔦", rarity: "special" });
            
            showNotification(`🔦 SELAMAT! Kamu mendapatkan FLASHLIGHT! Sekarang cari Miner Helm di Depth Gear untuk buka Mining!`, "success");
        } else if (recipe.output.type === "ticket" && recipe.output.id === "illuvatar") {
        ...
        gameData.mining.rocks = backupRocks;
...
function loadFishTank(rarity, tankId) {
    ...
    Object.keys(fishInTank).forEach(fishId => {
        ...
        if (fish) {
            ...
            fishElement.innerHTML = `<span class="${fishClass}">${fish.emoji}</span>`;
...
function switchMiningTab(tabId) {
    document.querySelectorAll('.mining-tab').forEach(tab => tab.classList.remove('active'));
    document.querySelectorAll('.mining-pane').forEach(pane => pane.classList.remove('active'));
    
    const activeTab = document.querySelector(`[data-mining-tab="${tabId}"]`);
    const activePane = document.getElementById(`mining-${tabId}`);
    
    if (activeTab) activeTab.classList.add('active');
    if (activePane) activePane.classList.add('active');
    
    if (tabId === 'main') loadMiningMain();
    else if (tabId === 'shop') loadMiningShop();
    else if (tabId === 'skill') loadMiningSkillTree();
    else if (tabId === 'exchange') loadMiningExchange();
}
    ...
    try {
        ...
        if (save) {
            ...
            if (saveData.gameData && saveData.gameData.version !== gameData.version) {
                ...
                showNotification("🔄 Game update ke v6! Fitur Mining ditambahkan!", "info");
                ...
                if (!gameData.specialItems.flashlight) {
                    gameData.specialItems.flashlight = false;
                }
                
                if (!gameData.depthGear.minerHelm) {
                    gameData.depthGear.minerHelm = false;
                }
                ...
                if (!gameData.skills.penawar) {
                    gameData.skills.penawar = { level: 0, bonus: 0 };
                }
                
                if (!gameData.skills.animalLovers) {
                    gameData.skills.animalLovers = { unlocked: false };
                }
                ...
                if (gameData.illuvatarTickets === undefined) {
                    gameData.illuvatarTickets = 0;
                }
            ...
            Object.assign(gameData, saveData.gameData);
            if (saveData.rods) Object.assign(rods, saveData.rods);
            if (saveData.baits) Object.assign(baits, saveData.baits);
            if (saveData.miningTools) Object.assign(miningTools, saveData.miningTools);
            if (saveData.quests) Object.assign(quests, saveData.quests);
            currentSpot = saveData.currentSpot || 0;
            currentDepth = saveData.currentDepth || "surface";
            caughtSecretSpots = saveData.caughtSecretSpots || {
                danau: false, kuil: false, laut: false, sungai: false, angkasa: false, brainrot: false, crypto: false, atlantis: false, valinor: false
            };
...
function setupEventListeners() {
    ...
    if (fishBtn) {
        ...
        console.log("✅ fish-btn listener attached");
    }
    
    if (gachaBtn) {
        gachaBtn.addEventListener('click', spinGacha);
        console.log("✅ gacha-btn listener attached");
    }
    ...
    document.querySelectorAll('.mining-tab').forEach(tab => {
        tab.addEventListener('click', () => switchMiningTab(tab.getAttribute('data-mining-tab')));
    });
    ...
    window.addEventListener('click', (e) => {
        ...
        if (miningModal && e.target === miningModal) miningModal.style.display = 'none';
    });
    ...
    if (miningBtn) miningBtn.addEventListener('click', switchToMining);
    if (backToMainBtn) backToMainBtn.addEventListener('click', switchToMain);
    if (backToMainFromMiningBtn) backToMainFromMiningBtn.addEventListener('click', switchToMainFromMining);
...
style.textContent = `
    ...
    .mining-tab.active { background: rgba(255, 255, 255, 0.2) !important; font-weight: bold !important; }
