...
const gameData = {
    ...
    rank: {
        ...
        losses: 0,
        totalBattles: 0,
        // NEW: Leaderboard position (0 = not in leaderboard)
        leaderboardPosition: 0,
        // NEW: Highest rank achieved
        highestRank: "Warrior"
    },
    updateSettings: {
...
function checkAtlantisSpawn() {
    ...
    if (!gameData.atlantis.active && now >= gameData.atlantis.nextSpawn) {
        ...
        gameData.atlantis.disappearTime = now + (5 * 60 * 1000);
        gameData.atlantis.nextSpawn = now + (5 * 60 * 1000);
        ...
        createSpotButtons();
        showNotification("🌊 ATLANTIS TERBUKA! Spot misterius muncul selama 5 menit!", "success");
    ...
    if (gameData.atlantis.active && now >= gameData.atlantis.disappearTime) {
        ...
        createSpotButtons();
        showNotification("🌊 Atlantis menghilang... Kembali dalam 5 menit!", "info");
    }
}
...
// ==================== NEW: LEADERBOARD & RANK FUNCTIONS ====================
function loadRankBattle() {
    ...
    const nextRankExp = rankInfo.maxExp;
    
    // Hitung leaderboard position
...
function generateLeaderboardHTML() {
    ...
    // Gabungkan NPC dan player (kalau masuk leaderboard)
    let allEntries = [...leaderboardNPCs];
    
    // Cek apakah player masuk leaderboard
    if (gameData.rank.leaderboardPosition > 0) {
        ...
        // Insert player di posisi yang benar
        allEntries.splice(gameData.rank.leaderboardPosition - 1, 0, playerEntry);
        // Hapus entry ke-6 kalau ada
        if (allEntries.length > 5) allEntries.pop();
    }
    
    // Urutkan berdasarkan exp (tertinggi ke terendah)
...
function updateLeaderboardPosition() {
    ...
    // Cek apakah player mengalahkan NPC ke-5
    if (playerExp > leaderboardNPCs[4].exp) {
        position = 5;
        // Cek apakah bisa naik lebih tinggi
        for (let i = 4; i >= 0; i--) {
