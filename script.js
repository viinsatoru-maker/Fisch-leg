...
function createSkillCard(skillKey, skillData) {
    ...
    card.innerHTML = `
        <div style="text-align: center;">
            ...
            ` : '<p style="color: #FFD700;">✨ MAX LEVEL</p>'}
        </div>
    `;
    ...
    rankContent.innerHTML = `
        <div style="padding: 20px;">
            ...
            <button onclick="document.getElementById('rank-modal').style.display='none'" 
                    style="width: 100%; padding: 10px; background: #666; border: none; border-radius: 6px; color: white; cursor: pointer;">
                BATAL
            </button>
        </div>
    `;
    ...
    luckDisplay.innerHTML = `
        <div style="text-align: center; padding: 15px; background: rgba(0,0,0,0.2); border-radius: 8px; margin: 15px 0;">
            ...
            <div style="font-size: 0.9rem; text-align: left;">
                <div style="margin: 5px 0;">🎣 Rod: ${rods[gameData.currentRod].name} (${rods[gameData.currentRod].luck}x)</div>
                <div style="margin: 5px 0;">🪱 Bait: ${baits[gameData.currentBait].name} (${baits[gameData.currentBait].luck}x)</div>
                <div style="margin: 5px 0;">🏠 Hut: +${gameData.village.hutLevel * 10}%</div>
                ${activePet ? `<div style="margin: 5px 0;">🐕 Pet: ${activePet.name} (${activePet.effect.type === 'luck_multiplier' ? activePet.effect.value + 'x' : 'Active'})</div>` : ''}
                <div style="margin: 5px 0;">🍀 Lucky Skill: +${gameData.skills.lucky.level * 100}%</div>
                ${gameData.activePotions.length > 0 ? 
                    `<div style="margin: 5px 0;">🧪 Potion: ${gameData.activePotions[0].name} (${gameData.activePotions[0].multiplier}x)</div>` : ''}
                <div style="margin: 5px 0;">🌤️ Weather: ${weather.effects[weather.current].luck}x</div>
            </div>
        </div>
    `;
}
...
function loadShop(category) {
    ...
    items.forEach(item => {
        ...
        // Gunakan emoji kustom jika ada, jika tidak gunakan default
        const emoji = item.emoji || (category === 'rods' ? '🎣' : '🪱');
        
        // Format harga untuk tampilan
        const priceDisplay = item.price > 0 ? item.price : (item.id === 0 ? 'Quest/Exchange Reward' : 'Quest Reward');
        ...
        itemCard.innerHTML = `
            <div style="background: rgba(255,255,255,0.05); padding: 15px; border-radius: 8px; margin: 5px; text-align: center;">
                ...
                <button class="buy-btn" ${isDisabled ? 'disabled' : ''} style="width: 100%; padding: 8px; background: ${isDisabled ? '#666' : '#4CAF50'}; border: none; border-radius: 6px; color: white; cursor: ${isDisabled ? 'not-allowed' : 'pointer'};">
                    ${buttonText}
                </button>
            </div>
        `;
...
function updatePotionDisplay() {
    ...
    gameData.activePotions.forEach(potion => {
        ...
        effectItem.style.background = 'rgba(255,255,255,0.05)';
        ...
        effectItem.style.border = '1px solid rgba(255,215,0,0.3)';
